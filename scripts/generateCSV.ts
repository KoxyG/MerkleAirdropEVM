...
function generateData(): { address: string|null, amount : number }[] {
    const data = [];
    for (let i = 1; i <= 500; i++) {
       
        const bal = Math.ceil(Math.random() * i * 17)
    
        data.push({ address: generateRandomAddress(), amount: bal });
    }
...
function convertToCSV(data: { address: string|null, amount : number }[]): string {
